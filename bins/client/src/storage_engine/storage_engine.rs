...
impl StorageEngine {
    pub fn new(database_path: impl Into<String>) -> StorageEngine {
        StorageEngine {
    ...
    fn db(&self) -> Result<&DatabaseConnection> {
        self.db
            .get()
            .ok_or_else(|| anyhow!("Unable to get database connection"))
    }
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        log::debug!("Database connection closed for: {}", self.database_path);
        Ok(())
    }
    ...
    pub async fn cleanup_job(&self, clean_older_than: DateTime<Utc>) -> Result<()> {
        ...
        partition_entry::Entity::delete_many()
            .filter(partition_entry::Column::CollectedAt.lt(clean_older_than))
            .exec(db)
            .await?;
    ...
    ) -> Result<Vec<(partition_entry::Model, Vec<partition_stats::Model>)>> {
        let db = self.db()?;
        let rows = partition_entry::Entity::find()
            .filter(partition_entry::Column::CollectedAt.between(start, end))
            .order_by_asc(partition_entry::Column::CollectedAt)
            .find_with_related(partition_stats::Entity)
            .all(db)
            .await?;
        Ok(rows)
    }
    ...
    pub async fn get_partition_stats_latest(
        &self,
        last_n: u64,
    ) -> Result<Vec<(partition_entry::Model, Vec<partition_stats::Model>)>> {
        let db = self.db()?;
        let latest_ids: Vec<i64> = partition_entry::Entity::find()
            .order_by_desc(partition_entry::Column::CollectedAt)
            .limit(last_n)
            .all(db)
            .await?
            .into_iter()
        ...
        let mut rows = partition_entry::Entity::find()
            .filter(partition_entry::Column::Id.is_in(latest_ids))
            .order_by_desc(partition_entry::Column::CollectedAt)
            .find_with_related(partition_stats::Entity)
            .all(db)
            .await?;
...
#[async_trait]
impl ErrorStoreStorageEngine for StorageEngine {
    async fn save_error(&self, error: Error) -> Result<()> {
        let db = self.db()?;
        let model = error::ActiveModel {
            file: Set(Some(error.call_site().file().clone())),
            line: Set(*error.call_site().line() as i64),
            severity: Set(Some(error.severity().to_string())),
            message: Set(Some(error.message().clone())),
            collected_at: Set(Utc::now().into()),
            ..Default::default()
        };
        error::Entity::insert(model).exec(db).await?;
        Ok(())
    }
}

#[async_trait]
impl SpeedtestStatsCollectionStorageEngine for StorageEngine {
...
#[async_trait]
impl ContainerStatsCollectionStorageEngine for StorageEngine {
...
#[async_trait]
impl BaseMetricCollectionStorageEngine for StorageEngine {
    async fn save_base_metrics(&self, base_metrics: BaseMetrics) -> Result<()> {
        ...
        if let Some(partitions) = base_metrics.disks
            && let Some(first_partition) = partitions.first()
        {
            let entry_model = partition_entry::ActiveModel {
                collected_at: Set(first_partition.collected_at.into()),
                ..Default::default()
            };
            //gets the id to reference it
            ...
            for partition in partitions {
                let model = partition_stats::ActiveModel {
                    partition_entry_id: Set(inserted_entry.last_insert_id),
                    name: Set(partition.name),
                    device: Set(partition.device),
                    mount_point: Set(partition.mount_point),
                    fs_type: Set(partition.fs_type),
                    total_bytes: Set(partition.total_bytes as i64),
                    used_bytes: Set(partition.used_bytes as i64),
                    available_bytes: Set(partition.available_bytes as i64),
                    used_blocks: Set(partition.used_blocks as i64),
                    available_blocks: Set(partition.available_blocks as i64),
                    block_size: Set(partition.block_size as i64),
                    collected_at: Set(partition.collected_at.into()),
                    ..Default::default()
                };
                partition_stats::Entity::insert(model).exec(db).await?;
            }
        }
...
#[async_trait]
impl DataCleanupStorageEngine for StorageEngine {
    async fn remove_all_older_than(&self, cutoff_time: DateTime<Utc>) -> Result<()> {
        let db = self.db()?;

        // no independent deletes needed for child tables (cpu_core_stats, partition_stats,
        // container_stats, process_stats) they cascade-delete via their FK's on_delete = "Cascade"
