...
impl MetricsFile {
        ...
        D: Serialize + for<'de> Deserialize<'de> + 'static + DataCreationTime,
    {
        ...
        // set the timestamps
        let first_metric_timestamp = self
            .blocks
            .as_ref()
            .ok_or(MetricsFileFormatError::HeaderDataTimeError(
                "no data entry found (should not happen)".to_string(),
            ))?
            .iter()
            .map(|b| b.data_creation_time())
            .min()
            .ok_or(MetricsFileFormatError::HeaderDataTimeError(
                "no first element found to set in the block header for first data block."
                    .to_string(),
            ))?;
...
mod tests {
    ...
    fn default_generation_test() {
        ...
        assert_eq!(
            bare_bone.header.checksum().clone(),
            bare_bone.header.compute_checksum()
        );
    ...
    fn generate_with_data_test() {
        ...
        // the test data
        let vec_data: Vec<TestMetric<Vec<u8>>> = data
            .into_iter()
            .enumerate()
            .map(|(i, e)| TestMetric {
                data: e,
